...
For calendar template backgrounds, put `background` inside each `BUSY` / `FREE` status block when the state needs full-screen color or layout-level colored regions. `background.bgColor` controls the full canvas outside `background.rectangle`; `background.rectangle` uses percentage coordinates and controls a large fill/stroke region before `items` are drawn.
Before finalizing calendar template JSON, run the practical layout checklist in `references/display-render-layout-knowledge.md`; pay special attention to dynamic `TEXT` fields such as `ROOM_NAME`, `UPCOMING_*_CREATOR`, and event summaries.
For `BITMAP_URI` items, do not include `data.id`; device testing shows some Displays fail to parse `BITMAP_URI` when an `id` field is present.
